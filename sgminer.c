...
static void gen_gbt_work(struct pool *pool, struct work *work)
{
  ...
  if (pool->algorithm.type == ALGO_CRE || pool->algorithm.type == ALGO_SCRYPT) {
    calc_midstate(work);
  }
  local_work++;
  work->pool = pool;
...
static bool work_decode_eth(struct pool *pool, struct work *work, json_t *val)
{
  ...
  if (!parse_diff_ethash((char*) &Target[0], (char*) &TgtStr[0]))
    goto out;
...
static bool work_decode(struct pool *pool, struct work *work, json_t *val)
{
  if (pool->algorithm.type == ALGO_ETHASH)
    return work_decode_eth(pool, work, val);
       ...
       char *hashshow, bool resubmit, char *worktime)
{
  ...
  struct cgpu_info *cgpu;
  double sharediff = work->work_difficulty;
  if (pool->algorithm.type == ALGO_ETHASH) sharediff /= 1e9;
  ...
  if (json_is_true(res) || (work->gbt && json_is_null(res))) {
    ...
    total_accepted++;
    pool->accepted++;
    cgpu->diff_accepted += sharediff;
    total_diff_accepted += sharediff;
    pool->diff_accepted += sharediff;
    mutex_unlock(&stats_lock);

    pool->seq_rejects = 0;
    cgpu->last_share_pool = pool->pool_no;
    cgpu->last_share_pool_time = time(NULL);
    cgpu->last_share_diff = sharediff;
    pool->last_share_time = cgpu->last_share_pool_time;
    pool->last_share_diff = sharediff;
    ...
    total_rejected++;
    pool->rejected++;
    cgpu->diff_rejected += sharediff;
    total_diff_rejected += sharediff;
    pool->diff_rejected += sharediff;
    pool->seq_rejects++;
    mutex_unlock(&stats_lock);
...
static bool submit_upstream_work(struct work *work, CURL *curl, char *curl_err_str, bool resubmit)
{
  ...
  if (work->pool->algorithm.type == ALGO_ETHASH)
  {
     ...
     char *ASCIINonce = bin2hex((unsigned char*) &tmp, 8);
  ...
  if (work->pool->algorithm.type == ALGO_NEOSCRYPT) datasize = 80;
    ...
    datasize = 192;
    be32enc(work->data + 180, 0x80000001UL);
    be32enc(work->data + 184, 0);
    be32enc(work->data + 188, 0x000005a0UL);
  }
...
static bool get_upstream_work(struct work *work, CURL *curl, char *curl_err_str)
{
  ...
  if (pool->algorithm.type == ALGO_ETHASH)
    pool->rpc_req = (char*) &eth_getwork_rpc[0];
...
static bool stale_work(struct work *work, bool share)
{
   ...
   * the nearest second */
  getwork_delay = (int) (pool->sgminer_pool_stats.getwork_wait_rolling * 5 + 1);
...
static void *stratum_sthread(void *userdata)
{
  ...
  while (42) {
    ...
    char votehex[16] = { 0 };
    char s[126 + 16 + 2048] = { 0 }; // todo: malloc
    struct stratum_share *sshare;
    ...
    data  =  (uint32_t *)work->data;

    sshare->sshare_time = time(NULL);
    // This work item is freed in parse_stratum_response
    sshare->work = work;
    mutex_lock(&sshare_lock);
    // Give the stratum share an unique id
    sshare->id = swork_id++;
    mutex_unlock(&sshare_lock);
    ...
    if (pool->algorithm.type == ALGO_ETHASH)
    {
      ...
      snprintf(s, sizeof(s),
        "{\"method\":\"mining.submit\",\"params\":[\"%s\",\"%s\",\"0x%s\",\"0x%s\",\"0x%s\"],\"id\":%d}",
        pool->rpc_user, work->job_id, ASCIINonce, ASCIIPoWHash, ASCIIMixHash, sshare->id);
      ...
      snprintf(s, sizeof(s),
        "{\"method\":\"mining.submit\",\"params\":[\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"%s],\"id\":%d}",
    ...
    while (time(NULL) < sshare->sshare_time + 120) {
      ...
      if (likely(stratum_send(pool, s, strlen(s)))) {
        ...
        ssdiff = (int) (sshare->sshare_sent - sshare->sshare_time);
...
retry_stratum:
  ...
  if (pool->algorithm.type == ALGO_ETHASH)
     pool->rpc_req = (char*) eth_getwork_rpc;
...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
  ...
  if (pool->algorithm.type == ALGO_ETHASH)
    return gen_stratum_work_eth(pool, work);
  ...
  * stratum diff when submitting shares */
  ...
  } else if (pool->algorithm.type == ALGO_CRE || pool->algorithm.type == ALGO_SCRYPT) {
    calc_midstate(work);
...
static void rebuild_nonce(struct work *work, uint32_t nonce)
{
  ...
  if (work->pool->algorithm.type == ALGO_DECRED) nonce_pos = 140;
...
static void update_work_stats(struct thr_info *thr, struct work *work)
{
  ...
  if (work->pool->algorithm.type == ALGO_ETHASH && test_diff && work->share_diff > test_diff)
    applog(LOG_DEBUG, "block check : %g diff %g", work->share_diff, test_diff); // to check
  else
